...
const SMSDataContextProvider = ({children}: Props) => {
  ...
  const classification_list = {
    ...
    '애완동물':
      '반려동물 반려동물분양 반려견분양 애견의류 동물병원', //동물카페 고양이카페 
    '여행,숙박':
      '호텔 콘도,리조트 레지던스 모텔 자연명소 자연공원 휴양림,산림욕장 문화,유적 문화재 기념물 릉,묘,총 관람,체험 아쿠아리움 투어버스 페리, 해운사',
    '음식점':
      '한식 중식 일식 베트남음식 인도음식 분식 술집 육류,고기요리 찌개,전골 국밥 국수 냉면 막국수 추어탕 감자탕 해장국 곰탕,설렁탕 갈비탕 두부요리 칼국수,만두 전,빈대떡 한정식 쌈밥 보리밥 비빔밥 죽 백반,가정식 돼지고기구이 피자 샌드위치 햄버거 핫도그 후렌치후라이 브런치 스테이크, 립 멕시코, 남미음식 이탈리아 음식 중식당 양꼬치 덮밥 일식당 일본식라면 돈가스 초밥, 롤 일식튀김, 꼬치 샤브샤브 우동,소바 카레 전통,민속주점 요리주점 맥주, 호프 바(BAR) 와인 이자카야 오뎅, 꼬치 포장마차 치킨,닭강정 토스트 스파게티,파스타전문',
    '카페':
  ...
  useEffect(() => {
    if (body.includes('승인') || !body.includes('취소')) {
      ...
      if (cname) {
        ...
        for (let i=0; i<13; i++){
          classification_list_values[i] = Object.values(classification_list)[i];
        }
        // 키워드 분류
        for (let i = 0; i < 13; i++){
          answer_keyword = '';
